...
# Shorten path: drive/root/~/project (collapse middle segments)
short_path=$(echo "$project_dir" | tr '\\' '/' | awk -F'/' '{
  if (NF >= 4) {
    printf "%s/%s/~/%s", $1, $2, $NF
  } else if (NF >= 2) {
    printf "%s/%s", $(NF-1), $NF
...
# Convert USD cents to KRW (1 USD = 1480 KRW)
KRW_RATE=1480
total_krw=$(awk "BEGIN {printf \"%.1f\", $total_cents * $KRW_RATE / 100}")
cost_display=$(printf "%s원" "$total_krw")
...
# Line break — chip 2 goes to second line
printf "\n"

# CHIP 2: model + context + cost + api time
printf "${FG_RIGHT}${CAP_LEFT}${RESET}"
printf "${BG_RIGHT}${BOLD}${FG_RIGHT_TEXT} ${ICON_BRAIN} %s ${ICON_MONITOR} %s %d%% ${ICON_DOLLAR} %s ${ICON_BOLT} %ss ${RESET}" "$model_display" "$bar" "$context_pct" "$cost_display" "$api_sec"
printf "${FG_RIGHT}${CAP_RIGHT}${RESET}"

# Line break — chips 3 & 4 go to second line
printf "\n"

# CHIP 3: git info (no surrounding spaces)
if [ -n "$git_branch" ]; then
    printf "${FG_MID}${CAP_LEFT}${RESET}"
    printf "${BG_MID}${BOLD}${FG_MID_TEXT} ${ICON_GITHUB} ${ICON_BRANCH} %s%s ${RESET}" "$git_branch" "$git_dirty"
...
printf " "
printf "${FG_MID}${CAP_LEFT}${RESET}"
printf "${BG_MID}${BOLD}${FG_MID_TEXT}${ICON_KEY} %s${RESET}" "$session_short"
printf "${FG_MID}${CAP_RIGHT}${RESET}"
